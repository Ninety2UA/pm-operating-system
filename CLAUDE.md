...
- **`core/scripts/validate.py`** — run `uv run core/scripts/validate.py` to check the framework for drift across 52 checks (frontmatter issues, broken cross-refs, MCP tool parity, workspace shape, pipeline artifact conformance, external CLI deps, bidirectional skill registry, MCP handler presence, BACKLOG.md structure, stale lock files, PIPELINE_STAGES parity, BOM, secret-bypass lint, backup coverage, tracked-but-ignored files, tracked-tree hygiene, adapter manifest). Inline `# /// script` metadata auto-installs pyyaml. Useful after bulk edits to skills/agents or before a release. Every subprocess it spawns is time-bounded and reports its own timeout by name, so a hung binary never reads as a check that passed. Non-blocking warnings are reported separately from hard failures; the documented classes are listed below.
...
- **Non-blocking warning classes** — the validator's documented warn-class findings are `pipeline-artifact`, `external-dep` (external CLI deps not on PATH), `lock-hygiene`, `ledger-link`, `live-registry`, `secret-bypass`, `backup-coverage`, `privacy-placement` (a tracked file the repository's own ignore rules cover), `tree-hygiene` (a tracked symlink or a case-folding path collision), and the `guard-wiring` timeout notice. Green means zero hard failures with warnings only in these classes; a warning in any other class is drift to fix, not noise. Environment notices such as a missing memory directory or `.mcp.json` describe the install, not the framework.
- **Local report mode** — `uv run core/scripts/validate.py --staleness-report` adds five warn-only sections over gitignored data: retired model IDs in project specs, watcher reports older than 14 days, projects whose Progress Log never records their status, unfilled template placeholders under `projects/` and `tasks/`, and synthesized artifacts whose `sources:` pointer is missing or names a file that no longer exists.
