...
If no `--model` flag is provided, default to the `perplexity_research` tool (perplexity MCP server).
...
- `--model deep` → use the `perplexity_research` tool (perplexity MCP server) (default)
