...
- the `list_tasks` tool (manager-ai MCP server) with `status: "d"` for completed tasks this week
- the `list_tasks` tool (manager-ai MCP server) with `status: "b"` for current blockers
- the `list_tasks` tool (manager-ai MCP server) with `status: "s"` for in-progress
- the `get_pipeline_status` tool (manager-ai MCP server) for project pipeline movement
- the `get_project_summary` tool (manager-ai MCP server) for active project state
...
Read template from `references/weekly-update-template.md` and fill in.
...
Show the memo to the user for review. Then offer (via structured questions):

| Option | Description |
|--------|-------------|
| Copy to clipboard | Output as plain text ready to paste into email |
| Post to Slack channel | Use the `slack_send_message` tool (Slack MCP server) (ask for channel) |
| Send as Slack DM | Use the `slack_send_message` tool (Slack MCP server) to a user |
