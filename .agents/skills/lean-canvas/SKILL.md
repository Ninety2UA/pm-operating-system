...
If no `--model` flag is provided, default to the `perplexity_ask` tool (perplexity MCP server).
...
Using the project context and research, fill in all 9 blocks. Read the template at `references/lean-canvas-template.md` and fill each section from the project context and research.
