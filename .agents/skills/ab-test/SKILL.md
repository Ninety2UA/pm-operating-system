...
source_sha256: 5053b1ee86b9654a0689c733ff55cc65fab417b04419f8f7c98cdc231bd45180
...
User: `/ab-test --plan`
Result: Collects hypothesis, baseline, MDE, α, power, traffic; computes required N per arm and expected duration; drafts a plan doc with Statistical Plan + Rollout Ramp + Kill Criteria + Observability from the template at `references/plan-template.md`. Prints the draft and offers to save to `knowledge/ab-plan-<slug>.md`.
...
### Inputs needed
Ask for (batched via structured questions if Claude Code surface supports it, else inline prompts):
...
Read `references/plan-template.md`, fill the 7 sections, and save to `knowledge/ab-plan-<slug>.md` (or return inline if the user declines the save prompt). Flag any unsupplied slot `[INFERRED]` so the doc remains usable.
...
Check the arguments provided when you were invoked for either:
...
Read the output template at `references/ab-test-output-template.md` and fill each section with the computed statistics from Step 3.
