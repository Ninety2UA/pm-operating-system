...
- the `list_meetings` tool (granola MCP server) — list recent meetings with metadata
- the `get_meetings` tool (granola MCP server) — fetch full content + enhanced notes for one or more meeting IDs
- the `get_meeting_transcript` tool (granola MCP server) — fetch raw transcript (paid plan only)
- the `list_meeting_folders` tool (granola MCP server) — browse meeting folders (paid plan only)
- the `query_granola_meetings` tool (granola MCP server) — natural-language Q&A across all meetings
...
- Try the `list_meetings` tool (granola MCP server) with a tiny window (e.g. `limit: 1`). If it succeeds, proceed.
- If the call fails with an auth error, OR the only Granola tools exposed are `authenticate` / `complete_authentication`, the user is NOT authenticated. Stop the sync and tell the user:
  ...
  > "Granola isn't authenticated in Claude. Open Claude Settings → Connectors → Granola to authenticate, OR call the `authenticate` tool (granola MCP server) → follow the flow → the `complete_authentication` tool (granola MCP server). Re-run `/meeting-sync` after auth succeeds."
...
Call the `list_meetings` tool (granola MCP server) with the cutoff. Filter to meetings with notes/content (skip placeholders).
...
Use structured questions (skip if `--all` or `--skip` was passed):
...
1. Call the `get_meetings` tool (granola MCP server) with the meeting ID(s) — returns title, attendees, time, enhanced notes.
2. Call the `get_meeting_transcript` tool (granola MCP server) with the meeting ID — paid plan only. If it errors with "subscription required", proceed without transcript.
...
Use structured questions per group:
...
- **Natural-language queries:** When the user asks about past meetings without specifying one (e.g., "What did we decide about pricing last week?"), call the `query_granola_meetings` tool (granola MCP server) instead of syncing — it answers across all meetings.
- **Folder browsing:** When the user wants to find meetings by folder/project, call the `list_meeting_folders` tool (granola MCP server) (paid plan only).
