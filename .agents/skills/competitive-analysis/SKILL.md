...
Default to the `perplexity_research` tool (perplexity MCP server) (deep) — competitive analysis needs depth.
...
Read the template at `references/competitive-analysis-template.md` and fill each section from the competitor research in Step 5 — it defines the full analysis structure (market landscape, competitor map, feature matrix, deep dives, pricing analysis, gaps, differentiation strategy, and battlecard).
