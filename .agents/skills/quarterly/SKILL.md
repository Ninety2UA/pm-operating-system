...
Call the `list_projects` tool (manager-ai MCP server) to get all projects. Identify:
...
If yes, format a concise summary: OKR scores, projects shipped/archived, top priorities for next quarter. Post via the `slack_send_message` tool (Slack MCP server). If Slack MCP is unavailable, skip silently.
