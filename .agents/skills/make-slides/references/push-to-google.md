...
- For editable/mixed/both: `build-native.js` at `references/build-native.js`
...
node references/build-native.js "$OUT_DIR" "$PRES_ID" --mode=editable
```
...
```bash
node references/build-native.js "$OUT_DIR" "$PRES_ID" --mode=mixed
```
...
PRES_EDIT=$(gws slides presentations create \
  ...
  | jq -r '.presentationId')
node references/build-native.js "$OUT_DIR" "$PRES_EDIT" --mode=editable
...
if [ -f "$OUT_DIR/meta.json" ] && jq -e '.google_slides.image_presentation_id // .google_slides.editable_presentation_id' "$OUT_DIR/meta.json" > /dev/null; then
  ...
  # Prompt the user before proceeding (via structured questions in the skill)
fi
```
