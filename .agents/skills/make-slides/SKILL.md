...
Read `references/design-rules.md` before building. It covers typography ladder, color discipline, layout patterns, chart guidance, and what NOT to do. The rules are opinionated — follow them unless the user overrides explicitly.
...
Full schema at `references/slides-spec.schema.md`. Five supported layouts: `hero`, `title-body`, `split`, `headline-chart`, `big-number`.
...
```bash
node references/render.js "$OUT_DIR"
```
...
Read `references/push-to-google.md` for the full API reference.
