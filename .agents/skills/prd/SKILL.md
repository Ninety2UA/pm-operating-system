...
source_sha256: 36474c28029f30a2ee7bbc73a37742a55a487b97adf39881af6791f9bb7d8dd9
...
flagged inline. Pass `--ask` when you want guided clarification via structured questions.
...
Calls structured questions once with up to 4 batched questions (pre-filled from idea.md
...
For a complete example, read `references/example-prd.md`.
...
2. **At most four constraint questions, each with a usable default.** Ask only what
   ...
   confirm]`; on `--ask`, batch the open ones into one extra structured questions call
...
**Opt-in path (`--ask` passed):** Call structured questions **once** with up to **4**
batched questions, each pre-filled from idea.md + GOALS.md so the user confirms
rather than retypes. The structured questions schema caps each call at 4 questions and
...
Then read `references/good-vs-bad.md` — it shows the bad→good form of
a Hypothesis, Problem/Evidence, Success Metric, and Scope line. Match the "good" column:
specific, numeric, falsifiable.

### Step 5: Write the PRD

Read the PRD template at `references/prd-template.md` and fill in
...
Before writing, read both `references/anti-patterns.md` (avoid each
of the 12 patterns) and `references/good-vs-bad.md` (match the
...
`references/anti-patterns.md` against the PRD you just wrote.
