...
# Cross-host caveat: the generated adapters carry no MCP tool allow-list, so on
# portable hosts this frontmatter does not enforce anything. There the read-only
# guarantee rests on the diagnostic-only rule stated in the body below and on the
# server refusing to move files without explicit consent (prune previews first).
